      ...
      IF (CTX.BUFLEN > 0) THEN
        ...
        FOR IDX IN 1..ADD LOOP
          ...
          IF (LEFT_OVER_MOD = 0) THEN
            CTX.BUFFER32(LEFT_OVER_BLK) := BITAND(CTX.BUFFER32(LEFT_OVER_BLK),BITS_00FFFFFF) + TO_NUMBER(UTL_RAW.CAST_TO_BINARY_INTEGER(UTL_RAW.SUBSTR(T_BUFFER,IDX,1)))*16777216;
          ELSIF (LEFT_OVER_MOD = 1) THEN
            CTX.BUFFER32(LEFT_OVER_BLK) := BITAND(CTX.BUFFER32(LEFT_OVER_BLK),BITS_FF00FFFF) + TO_NUMBER(UTL_RAW.CAST_TO_BINARY_INTEGER(UTL_RAW.SUBSTR(T_BUFFER,IDX,1)))*65536;
          ELSIF (LEFT_OVER_MOD = 2) THEN
            CTX.BUFFER32(LEFT_OVER_BLK) := BITAND(CTX.BUFFER32(LEFT_OVER_BLK),BITS_FFFF00FF) + TO_NUMBER(UTL_RAW.CAST_TO_BINARY_INTEGER(UTL_RAW.SUBSTR(T_BUFFER,IDX,1)))*256;
          ELSE
            CTX.BUFFER32(LEFT_OVER_BLK) := BITAND(CTX.BUFFER32(LEFT_OVER_BLK),BITS_FFFFFF00) + TO_NUMBER(UTL_RAW.CAST_TO_BINARY_INTEGER(UTL_RAW.SUBSTR(T_BUFFER,IDX,1)));
          END IF;
        END LOOP;
        ...
        BEGIN
          ...
          LOOP
            ...
            IF (TARGET_MOD            =0) THEN
              X_BUFFER32(TARGET_BLK) := BITAND(X_BUFFER32(TARGET_BLK),BITS_00FFFFFF) + TO_NUMBER(UTL_RAW.CAST_TO_BINARY_INTEGER(UTL_RAW.SUBSTR(T_BUFFER,IDX,1)))*16777216;
            ELSIF (TARGET_MOD         =1) THEN
              X_BUFFER32(TARGET_BLK) := BITAND(X_BUFFER32(TARGET_BLK),BITS_FF00FFFF) + TO_NUMBER(UTL_RAW.CAST_TO_BINARY_INTEGER(UTL_RAW.SUBSTR(T_BUFFER,IDX,1)))*65536;
            ELSIF (TARGET_MOD         =2) THEN
              X_BUFFER32(TARGET_BLK) := BITAND(X_BUFFER32(TARGET_BLK),BITS_FFFF00FF) + TO_NUMBER(UTL_RAW.CAST_TO_BINARY_INTEGER(UTL_RAW.SUBSTR(T_BUFFER,IDX,1)))*256;
            ELSE
              X_BUFFER32(TARGET_BLK) := BITAND(X_BUFFER32(TARGET_BLK),BITS_FFFFFF00) + TO_NUMBER(UTL_RAW.CAST_TO_BINARY_INTEGER(UTL_RAW.SUBSTR(T_BUFFER,IDX,1)));
            END IF;
          END LOOP;
